...
```{r}
load("~/Dropbox/CDS/Transitivity/TransitivityOutputs/TransitivityDataOrgWorkspace021115.RData")
...
RT.df$Task <- as.factor(RT.df$Task)

ggplot(data = RT.df[RT.df$RT>0,], aes(x = Trialnumber, y = RT, group = Group, col = Group))+
  geom_smooth(method = "loess")+
  theme_classic()+
  facet_grid(~Task)+
  xlab("Trial number")

rt.lmer1 <- lmerTest::lmer(RT ~ Group*Task*Trialnumber + (1|f.id), data = RT.df[RT.df$RT>0,])
...
(see above for rest of the stats in this section (e.g. model comparison for lmer and Bartlett test))
...
```{r}
mem.lmer <- lmerTest::lmer(IntransTripleCounted ~ (1|f.id) + c.Trialnumber*c.TrialQuad*Group, data=choice2.trial.data)
...
#Plot S1
ggplot(data=choice2.trial.data, aes(x = Trialnumber, y = IntransTripleCounted, group = Group, col = Group))+
  geom_smooth(method = "loess") +
  theme_classic()+
...
a4 <- lmerTest::lmer(IntransTripleCounted ~ RT.c + RT.quad.c + Group + RT.c:Group + (1 | f.id), data=choice2.trial.data)
summary(a4.lmerTest)
...
#Mediation?
med.lmer.c <-  lmerTest::lmer(IntransTripleCounted ~ (1|f.id) + Group +Trialnumber, data=choice2.trial.data[choice2.trial.data$RT>0,])
summary(med.lmer.c)

med.lmer.a <- lmerTest::lmer(RT ~ (1|f.id) + Group + Trialnumber, data=choice2.trial.data[choice2.trial.data$RT>0,])
summary(med.lmer.a)

med.lmer.b <- lmerTest::lmer(IntransTripleCounted ~ (1|f.id) + Group + RT + Trialnumber, data=choice2.trial.data[choice2.trial.data$RT>0,])
summary(med.lmer.b)
